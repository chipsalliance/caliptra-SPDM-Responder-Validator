...
local reqres_types = {
    ...
    [0x85] = "Request: CHUNK_SEND", -- 1.2
    [0x86] = "Request: CHUNK_GET", -- 1.2
    [0x87] = "Request: GET_ENDPOINT_INFO", -- 1.3
    [0xE0] = "Request: GET_MEASUREMENTS", -- 1.0.2
    [0xE1] = "Request: GET_CAPABILITIES", -- 1.0.2
    [0xE2] = "Request: GET_SUPPORTED_EVENT_TYPES", -- 1.3
    [0xE3] = "Request: NEGOTIATE_ALGORITHMS", -- 1.0.2
    [0xE4] = "Request: KEY_EXCHANGE", -- 1.1.0
    [0xE5] = "Request: FINISH", -- 1.1.0
    [0xED] = "Request: GET_CSR", -- 1.2
    [0xEE] = "Request: SET_CERTIFICATE", -- 1.2
    ...
    [0x05] = "Respond: CHUNK_SEND_ACK", -- 1.2
    [0x06] = "Respond: CHUNK_RESPONSE", -- 1.2
    [0x07] = "Respond: ENDPOINT_INFO", -- 1.3
    [0x60] = "Respond: MEASUREMENTS", -- 1.0.2
    [0x61] = "Respond: CAPABILITIES", -- 1.0.2
    [0x62] = "Respond: SUPPORTED_EVENT_TYPES", -- 1.3
    [0x63] = "Respond: ALGORITHMS", -- 1.0.2
    [0x64] = "Respond: KEY_EXCHANGE_RSP", -- 1.1.0
    [0x65] = "Respond: FINISH_RSP", -- 1.1.0
    [0x6D] = "Respond: CSR", -- 1.2
    [0x6E] = "Respond: SET_CERTIFICATE_RSP", -- 1.2
    [0x7C] = "Respond: KEY_PAIR_INFO", -- 1.3
    [0x7D] = "Respond: SET_KEY_PAIR_INFO_ACK", -- 1.3
...
local Errors = {
    [0x00] = "Reserved",
    [0x01] = "Invalid Request: One or more request fields are invalid",
    [0x02] = "Invalid Session: Record layer used an invalid session ID",
    [0x03] = "Busy: Responder is busy, send the message again",
    [0x04] = "Unexpected Request",
    [0x05] = "Unspecified Error",
    [0x06] = "Decrypt Error",
    [0x07] = "Unsupported Request",
    [0x08] = "Request In Flight",
    [0x09] = "Invalid Response Code",
    [0x0A] = "Session Limit Exceeded",
    [0x0B] = "Session Required",
    [0x0C] = "Reset Required",
    [0x0D] = "Response Too Large",
    [0x0E] = "Request Too Large",
    [0x0F] = "Large Response (Chunking Required)",
    [0x10] = "Message Lost",
    [0x11] = "Invalid Policy",
    [0x12] = "Data Too Large",
    [0x41] = "Version Mismatch: SPDM Major Version not supported",
    [0x42] = "Response Not Ready",
    [0x43] = "Request Resynch Required",
    [0x44] = "Operation Failed",
    [0x45] = "No Pending Requests",
    [0x46] = "Request Session Terminated",
    [0x47] = "Invalid State",
    [0xFF] = "Vendor/Other Standards Defined Error"
}
...
ErrorCode           = ProtoField.uint8("ErrorCode", "Error Code", base.HEX, Errors)
ErrorData           = ProtoField.uint8("ErrorData", "Error Data", base.HEX)
ChunkHandle         = ProtoField.uint8("ChunkHandle", "Chunk Handle", base.HEX)
ChunkSeqNo          = ProtoField.uint16("ChunkSeqNo", "Chunk Sequence Number", base.DEC)

RDTExponent         = ProtoField.uint8("RDTExponent", "RDT Exponent", base.DEC)
RequestCode         = ProtoField.uint8("RequestCode", "Request Code", base.DEC)
Token               = ProtoField.uint8("Token", "Token", base.DEC)
RDTM                = ProtoField.uint8("RDTM", "RDTM", base.DEC)

OpaqueErrorData     = ProtoField.bytes("OpaqueErrorData", "Opaque Error Data")
...
function spdm.dissector(buffer, pinfo, tree)
    ...
    local header_length = nil
    local is_spdm_type = false

    if length >= 9 and buffer(0, 4):uint() == 0x1 then
        if length == 4 then
        ...
        header_length = 9
        if bit.band(buffer(8, 1):uint(), 0x7F) == 0x5 then
            is_spdm_type = true
        end
    elseif length >= 5 and bit.band(buffer(4, 1):uint(), 0x7F) == 0x5 then
        local subtree_1 = tree:add(mctp, buffer(0, 5), "Management Component Transport Protocol Data")
        subtree_1:add(HDR_Version,  buffer(0, 1))
        subtree_1:add(Dest_ID,      buffer(1, 1))
        subtree_1:add(Src_ID,       buffer(2, 1))
        subtree_1:add(SOM,          buffer(3, 1))
        subtree_1:add(EOM,          buffer(3, 1))
        subtree_1:add(Pkt_SQ,       buffer(3, 1))
        subtree_1:add(TO,           buffer(3, 1))
        subtree_1:add(Tag,          buffer(3, 1))
        subtree_1:add(IC,           buffer(4, 1))
        subtree_1:add(Type,         buffer(4, 1))
        header_length = 5
        is_spdm_type = true
    elseif bit.band(buffer(0, 1):uint(), 0x7F) == 0x5 then
        ...
        header_length = 1
        is_spdm_type = true
    elseif bit.band(buffer(0, 1):uint(), 0xF0) == 0x10 then
        header_length = 0
        is_spdm_type = true
    end

    if not header_length or not is_spdm_type then
        return
    end
        pinfo.cols.protocol = spdm.name

        local subtree_2 = tree:add(spdm, buffer(header_length, length - header_length), "Security Protocol Data Model")

        subtree_2:add(Major,  buffer(header_length,     1))
        subtree_2:add(Minor,  buffer(header_length,     1))
        subtree_2:add(ReqRes, buffer(header_length + 1, 1))

        local info = buffer(header_length + 1, 1):uint()
        local p1   = buffer(header_length + 2, 1)
        local p2   = buffer(header_length + 3, 1)

        subtree_2:add(Param1, p1)
        subtree_2:add(Param2, p2)

        local begin = header_length + 4
        ...
        elseif info == 0xE4 then
            ...
            local key_ex = subtree_2:add(spdm, buffer(begin, length - begin), "Key Exchange Message")
            ...
            key_ex:add_le(ExcData,           buffer(begin + 36, D))
        ...
        elseif info == 0x86 then
            pinfo.cols.info = "Request: CHUNK_GET"
            local chunk_get = subtree_2:add(spdm, buffer(begin, length - begin), "Chunk Get Message")
            chunk_get:add(ChunkHandle, p2)
            if length - begin >= 2 then
                chunk_get:add_le(ChunkSeqNo, buffer(begin, 2))
            end

        elseif info == 0x85 then
            pinfo.cols.info = "Request: CHUNK_SEND"
            local chunk_send = subtree_2:add(spdm, buffer(begin, length - begin), "Chunk Send Message")
            chunk_send:add(ChunkHandle, p2)
            if length - begin >= 2 then
                chunk_send:add_le(ChunkSeqNo, buffer(begin, 2))
            end

        elseif info == 0x06 then
            pinfo.cols.info = "Respond: CHUNK_RESPONSE"
            local chunk_rsp = subtree_2:add(spdm, buffer(begin, length - begin), "Chunk Response Message")
            chunk_rsp:add(ChunkHandle, p2)
            if length - begin >= 2 then
                chunk_rsp:add_le(ChunkSeqNo, buffer(begin, 2))
            end
        ...
        elseif info == 0x04 then
            ...
            local n = buffer(begin + 1, 1):uint()

            local get_ver = subtree_2:add(spdm, buffer(begin, length - begin), "Version Message")
        ...
        elseif info == 0x7F then
            local code = buffer(begin - 2, 1):uint()
            local err_name = Errors[code] or "ERROR"
            pinfo.cols.info = "Respond: ERROR (" .. err_name .. ")"

            if length - begin == 0 then
                local err = subtree_2:add(spdm, buffer(begin - 2, 2), "Error Message")
                err:add_expert_info(PI_PROTOCOL, PI_ERROR, "SPDM Respond Error: " .. err_name)

                err:add(ErrorCode, buffer(begin - 2, 1))
                err:add(ErrorData, buffer(begin - 1, 1))

            else
                local err = subtree_2:add(spdm, buffer(begin - 2, length - begin + 2), "Error Message")
                err:add_expert_info(PI_PROTOCOL, PI_ERROR, "SPDM Respond Error: " .. err_name)
                ...
                if code == 0x0F then
                    local lr = err:add(spdm, buffer(begin, length - begin), "Error: Large Response (Chunking Required)")
                    if length - begin >= 1 then
                        lr:add(ChunkHandle, buffer(begin, 1))
                    end
                elseif code == 0x42 then
                    local resnotready = err:add(spdm, buffer(begin, length - begin), "Error: Response Not Ready")

                    resnotready:add(RDTExponent, buffer(begin, 1))
                    resnotready:add(RequestCode, buffer(begin + 1, 1))
                    resnotready:add(Token, buffer(begin + 2, 1))
                    resnotready:add(RDTM, buffer(begin + 3, 1))
                elseif code == 0xFF then
                    local vendorDefined = err:add(spdm, buffer(begin, length - begin), "Error: Vendor/Other Standards Defined")

                    local vendorid_len = buffer(begin, 1):uint()

                    vendorDefined:add   (Len,                     buffer(begin, 1))
                    vendorDefined:add_le(VendorID, buffer(begin + 1, vendorid_len))
                    vendorDefined:add_le(OpaqueErrorData, buffer(begin + 1 + vendorid_len, length - begin - vendorid_len - 1))
                end
            end
...
if udp_port then
    udp_port:add(2323, spdm)
end

local wtap_encap = DissectorTable.get("wtap_encap")
if wtap_encap then
    wtap_encap:add(45, spdm)
    if wtap.USER0 then
        wtap_encap:add(wtap.USER0, spdm)
    end
end

local mctp_type = DissectorTable.get("mctp.type")
if mctp_type then
    mctp_type:add(0x05, spdm)
end
