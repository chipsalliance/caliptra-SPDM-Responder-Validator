...
void spdm_test_case_algorithms_success_14 (void *test_context)
{
     ...
     * kem_alg only has one bit at most.
     * If KEY_EX_CAP==1 then kem_alg may be one valid ML-KEM algo or 0.
     * If KEY_EX_CAP==0 then kem_alg must be 0 or absent. */
    if (kem_alg == 0xFFFF) {
        /* more than one bit set -- fail */
        test_result = COMMON_TEST_RESULT_FAIL;
    } else if (((test_buffer->rsp_cap_flags & SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_KEY_EX_CAP) ==
                0) &&
               (kem_alg != 0x0)) {
        /* no KEY_EX_CAP but kem_alg is non-zero -- fail */
        test_result = COMMON_TEST_RESULT_FAIL;
    } else {
        test_result = COMMON_TEST_RESULT_PASS;
    }
